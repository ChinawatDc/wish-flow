import { adminResetSecurityPin } from "@/lib/account-security-service";
...
/** Admin reset Security PIN ของ user อื่น — ต้องผ่าน step-up; PIN ชั่วคราวโชว์ครั้งเดียว */
export async function POST(request: Request, { params }: Params) {
  try {
    const admin = await requireAdminStepUp();
    const { id } = await params;

    const result = await adminResetSecurityPin({
      actor: admin,
      targetId: id,
    ...
    if ("error" in result) {
      const map: Record<string, { msg: string; status: number }> = {
        self_reset_blocked: {
          msg: "ไม่สามารถรีเซ็ต PIN ตัวเองได้ — ใช้หน้าโปรไฟล์แทน",
          status: 400,
        },
    ...
    return jsonOk(
      { ok: true, tempPin: result.tempPin },
